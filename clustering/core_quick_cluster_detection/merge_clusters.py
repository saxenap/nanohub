...
def merge_clusters(name, geocache, userToGeo, clusters, clusterFileName, createIntermediateFiles):
	intersections = generateClusterIntersections(clusters)
	print(sys.stderr, name, "cs", len(clusters), "is", len(intersections))
	iperf = runIntersectionAffinity(clusters, intersections)
	print(sys.stderr, name, "iperf",iperf, "cs", len(clusters), "is", len(intersections))
	cleanUpRemainingIntersections(clusters, intersections)
	cleanUpRemainingClusters(clusters, 5)
	if clusterFileName != None and createIntermediateFiles == True:
		writeClusterFile(clusters, clusterFileName)
	intersections = generateClusterIntersections(clusters)
