...
def core_classroom_analysis(inparams):
    ...
    print("intra_tool_cluster_df:\n", intra_tool_cluster_df)
    print("students_info_df:\n", students_info_df)
    print("class_info_df:\n", class_info_df)
    print("classtool_info_df:\n", classtool_info_df)
    ...
    # xufeng's original algorithm to CSV file
    intra_tool_cluster_df.to_csv(os.path.join(inparams.scratch_dir, 'cp1_intra_tool_cluster_df.csv'))
    # class_info_df.to_csv(os.path.join(inparams.scratch_dir, 'class_info_df.csv'))
    # print(os.path.join(inparams.scratch_dir, 'cp1_intra_tool_cluster_df.csv'))
    # for example: /home/users/shang26/xufeng-clustering/temp/cp1_intra_tool_cluster_df.csv


    # transform output to a csv file that has one cluster per line (to match the output in cost_cluster_analysis, for raindrop purpose)

    import csv
    with open(os.path.join(inparams.scratch_dir, 'cp1_intra_tool_cluster_df.csv'), 'r') as csv_file:
        cp1 = csv.reader(csv_file)
        next(cp1)
        outfile_name = 'xu_v1_' + inparams.data_probe_range[0].strftime("%Y_%m_%d") + '-' + \
                       inparams.data_probe_range[1].strftime("%Y_%m_%d") + '.csv'
        number_of_users = []
        final_clusters = []
        with open(os.path.join(inparams.scratch_dir, outfile_name), 'w') as f:
            for cluster in cp1:
                name_str_arr = cluster[7][1:-1].split(', ')
                number_of_users.append(len(name_str_arr))
                # final_clusters.append(set(name_str_arr))
                final_clusters.append(set([x.replace('\'', '') for x in name_str_arr]))
                for i in range(len(name_str_arr)):
                    if i != len(name_str_arr) - 1:
                        print(name_str_arr[i][1:-1] + ',', end='', file=f)
                    else:
                        print(name_str_arr[i][1:-1], end='', file=f)
                print('', file=f)
    ...
    number_of_clusters = len(intra_tool_cluster_df)
    print(number_of_users)
    print(number_of_clusters)
    print(final_clusters)
    return final_clusters, number_of_clusters, number_of_users

    # # combine clusters that share the same class_id in intra_tool_cluster_df
    # num_class_id = intra_tool_cluster_df['class_id'].max() + 1
    # class_id_to_user = [None] * num_class_id
    #
    # # comment these two lines if want to save the cluster file
    # # for _, row in intra_tool_cluster_df.iterrows():
    # #     class_id_to_user[row['class_id']] = row['user_set']
    #
    # # uncomment below if want to see the cluster file
    # outfile_name = 'xu_v2_' + inparams.data_probe_range[0].strftime("%Y_%m_%d") + '-' + \
    #                inparams.data_probe_range[1].strftime("%Y_%m_%d") + '.csv'
    # with open(os.path.join(inparams.scratch_dir, outfile_name), 'w') as f:
    #     for _, row in intra_tool_cluster_df.iterrows():
    #         i = 0
    #         class_id_to_user[row['class_id']] = row['user_set']
    #         for name in row['user_set']:
    #             if i != len(row['user_set']) - 1:
    ...
    # count number of clusters and number of users in each cluster
    # number_of_clusters = len(class_id_to_user)
    # number_of_users = [len(v) for v in class_id_to_user]
    # # print(number_of_users)
    # # print(number_of_clusters)
    # # print(class_id_to_user)
    # return class_id_to_user, number_of_clusters, number_of_users
