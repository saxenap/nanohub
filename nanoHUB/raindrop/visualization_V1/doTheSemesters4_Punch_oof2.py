...
rootdir = "CLUSTERSBYSEMESTER_TOOLSTART_OOF2"
hostname = "db.nanohub.org"
username = "hub_read"
password = "XXXXXX"
port = "3306"
databasename = "nanohub_metrics"

geoFileName = "data/users_geoclustered_protocol_5-6-7_on_or_after_2007-01-01.csv"
semesters = [
...
for name, start, end in semesters:
	...
	comd = "mysql -h " + hostname + " -P " + port + " -u " + username + " -p" + password + " --batch --execute \"select distinct user, jtv.toolname, to_days(datetime) as tds from toolstart as sl, metrics_tool_version as jtv where jtv.instance = sl.tool and jtv.toolname = 'oof2' and protocol in (5,6,7) and datetime > '" + start + "' and datetime < '" + end + "' and user <> '' order by user, tds;\" " + databasename + " >> " + rootdir + "/" + name + "/userXtoolXday.csv"
	print comd
