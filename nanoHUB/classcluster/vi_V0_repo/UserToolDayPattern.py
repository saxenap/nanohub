...
class UserToolDayPattern:
	...
	def grabFromDatabase(self, connection):
		sql = """
			select distinct username, jtv.toolname, to_days(start) as tds from sessionlog as sl, nanohub_22022011.jos_tool_version as jtv
				where
			username = %s and jtv.instance = sl.appname and start > '2000-07-01 00:00:00' and start < '2011-01-01 00:00:00' and username <> ''
			order by username, tds;
			"""
...
class UserToolDayPatternList:
	...
	def __init__(self, showCohort):
		#startdate and enddate are only important when using a db.  
		#If reading from a file, they are unimportant.
		self._startdate = '0000-00-00'
		self._enddate = '0000-00-00'
		self._showCohort = showCohort
		self._minDay = -1
		self._maxDay = -1
		self._userPatterns = {}
	...
	def getUserPattern(self, username):
		return self._userPatterns[username]
	...
	def grabAllFromDatabase(self, connection):
		sql = """
			select distinct username, jtv.toolname, to_days(start) as tds from sessionlog as sl, nanohub_22022011.jos_tool_version as jtv
				where
			jtv.instance = sl.appname and start > %s and start < %s and username <> ''
			order by username, tds;
			"""
		cursor = connection.cursor()
	...
	def grabAllFromDatabaseUsingToolstartTable(self, connection):
		sql = """
			select distinct user, jtv.toolname, to_days(datetime) as tds from toolstart as sl, metrics_tool_version as jtv
				where
			jtv.instance = sl.tool and datetime > %s and datetime < %s and user <> ''
			order by user, tds;
			"""
		cursor = connection.cursor()
		cursor.execute(sql, (self._startdate, self._enddate))
	...
	def incorporatePlopWord(self, word, separatorUser, rv):
		...
		if len(locArray) > 0:
			for i in xrange(0, len(locArray[0])):
	...
	def getGeoLabelForCluster(self, cluster, userToGeo):
		...
		for user in cluster:
			...
			if userGeo not in geoTriplets:
				if user in userToGeo: geoTriplets[userGeo] = userToGeo[user][1]
	...
	def getDummyPattern(self):
		...
		for i in xrange(self._minDay, self._maxDay+1):
	...
	def getOrderedUserList(self, clusters, geographicClusters, userToGeo, plopWords):
		...
		for cluster in clusters:
			...
			if geoTriplet[0] == "WEST LAFAYETTE" or geoTriplet[0] == "URBANA" or geoTriplet[0] == "EVANSTON" or True:
				...
				if plopWords: self.incorporatePlopWord(clusterGeoLabel, "RESET", rv)
		...
		if self._showCohort:
			for cluster in geographicClusters:
				#clusterGeoLabel, primaryGeography, geoTriplet = self.getGeoLabelForCluster(cluster, userToGeo)
				if cluster[0].startswith("WEST LAFAYETTE") or cluster[0].startswith("URBANA") or cluster[0].startswith("EVANSTON") or True:
					rv.append(("GEO", self.getDummyPattern()))
					if plopWords: self.incorporatePlopWord(cluster[0], "GEO", rv)
					print cluster[0]
					cluster[1].sort(key=lambda usr: self._userPatterns[usr].getEarliestDayRepresented() if usr in self._userPatterns else 0)
...
#		im = Image.new("RGB",(100,100))
		...
		im = Image.new("RGB",((self._maxDay - self._minDay + 1) * (wPixelsPerUse + wLPixelBufferPerEvent + wRPixelBufferPerEvent),
			...
			for day in userPattern.patterns:
				...
				if ('PUBLICATION SUBMITTED' not in toolsUsed and
					...
					if 'dummy' not in toolsUsed:
						...
						height = hPixelsPerUse
				curColor = 0
				for h in xrange(upperRight[1],upperRight[1] + height):
					for w in xrange(upperRight[0],upperRight[0] + width):
	...
	def makeClusterUserToolDayClustersizeList(self, clusters, geocache, userToGeo):
		...
		for cluster in clusters:
			...
			for user in cluster:
				...
				userPattern = self._userPatterns[user]
